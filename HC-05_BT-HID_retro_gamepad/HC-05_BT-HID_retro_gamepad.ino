...
/*
    ...
    Потребление:
      В режиме поиска соединения ~18-40 мА
      В рабочем режиме 25-35 мА
      В режиме сна на ATMEG'е с 8МГц менее 10 мкА / на LGT8F328P ~100 мкА
    ...
    Возможности:
      ...
      Глубокий сон BT-модуля и Arduino по достижению таймера бездействия или по нажатию соотв. комбинации -
        - поверх кнопок действия мысленно рисуется наконечник стрелы смотрящий вверх:
        (SEGA - "ВНИЗ + X + Z + B"; NES - "ВНИЗ + B + A"); SNES - "ВНИЗ + Y + B + A") в течение 3 сек.
      Смена режима (Стандартный/Тубро/Слоумо) по комбинации (в дополнение световая индикация) -
        поверх кнопок действия мысленно рисуется наконечник стрелы смотрящий вниз:
        (SEGA - "ВВЕРХ + A + Y + C";; NES - "ВВЕРХ + B + A"); SNES - "ВВЕРХ + Y + X + A") в течение 3 сек.
...
PowerManager gamepad_power_manager( digitalPinToInterrupt( getButtonPin(RN42_HID_gamepad::Button::START) ) );
...
void onCombinationTimeout()
{
  ...
  if ( hid_gamepad.isPowerOffCombination() )
  {
    ...
    gamepad_power_manager.wake();   // Вызовет колбэк isrWakeUp()
  }
